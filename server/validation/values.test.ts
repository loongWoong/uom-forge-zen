...
import { validateCompiledModelWithMeta } from './compiled-model.ts'
import {
  maxDocumentChars,
  DEFAULT_MAX_DOCUMENT_CHARS,
} from './document.ts'

test('parseJsonOutput extracts the JSON object from prose or code fences', () => {
  assert.deepEqual(parseJsonOutput('```json\n{"a":1}\n```'), { a: 1 })
  assert.deepEqual(parseJsonOutput('先输出了一段规划文字。\n{"a":{"b":"文本"}}\n后缀说明'), {
    a: { b: '文本' },
  })
  // 字符串字面量里的括号不能干扰配平扫描
  assert.deepEqual(parseJsonOutput('说明 { 干扰\n{"q":"含 } 括号"}'), { q: '含 } 括号' })
})
...
test('parseJsonOutputWithMeta repairs truncated JSON and reports the recovery', () => {
  ...
  // 悬空的键名与冒号也要能闭合
  assert.deepEqual(parseJsonOutputWithMeta('{"a": 1, "b"').value, { a: 1, b: null })
  assert.deepEqual(parseJsonOutputWithMeta('{"a":').value, {})
  assert.deepEqual(parseJsonOutputWithMeta('{"list": [1, 2').value, { list: [1, 2] })
...
test('compile-stage recovery notices surface as validation warnings', () => {
  const model = {
    schemaVersion: '1',
    name: '领域',
    summary: '摘要',
    objects: [
      { id: 'o1', name: '主体', description: '执行者', properties: [], evidence: [] },
    ],
    relations: [],
    actions: [],
    functions: [],
    ...
    activities: [
      {
        ...
        evidence: [],
        requirements: [
          { description: '要求', elements: ['o1'], status: 'partial', reason: '待支撑评估', evidence: [] },
        ],
      },
    ],
    boundaries: [],
  ...
  assert.ok(notices.some((notice) => notice.includes('截断')))
  // 干净输出不产生恢复提示
  assert.deepEqual(validateCompiledModelWithMeta(JSON.stringify(model)).notices, [])
})
