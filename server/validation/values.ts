...
export function parseJsonOutputWithMeta(
  ...
  const consider = (candidate: string | null, kind: 'extract' | 'repair'): void => {
    if (!candidate) return
    const exact = tryParse(candidate)
    if (exact.ok) {
      candidates.push({ value: exact.value, notices: [], length: candidate.length, kind, cleaned: false })
      return
    }
    const loosened = tryParse(candidate.replace(/,\s*([}\]])/g, '$1'))
    if (loosened.ok)
      candidates.push({ value: loosened.value, notices: [], length: candidate.length, kind, cleaned: true })
  }
...
export function parseJsonOutput(text: string, label = '分析结果'): unknown {
  return parseJsonOutputWithMeta(text, label).value
}
