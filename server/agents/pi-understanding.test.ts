...
test('independent coverage check treats omitted and partial blocks as gaps', async () => {
  const runTurn = async () =>
    JSON.stringify({
      coverage: [
        { blockId: 'block-1', status: 'complete', note: '已说明' },
        { blockId: 'block-2', status: 'partial', note: '遗漏条件' },
      ],
    })
  ...
  assert.deepEqual(gaps.map(({ id, status }) => [id, status]), [
    ['block-2', 'partial'],
    ['block-3', 'missing'],
  ])
})
...
test('independent coverage check accepts only complete known blocks', async () => {
  const runTurn = async () =>
    JSON.stringify({
      coverage: [{ blockId: 'block-1', status: 'complete', note: '已说明' }],
    })
  const gaps = await independentlyCheck(
    '说明',
    [{ id: 'block-1', text: '事实一' }],
